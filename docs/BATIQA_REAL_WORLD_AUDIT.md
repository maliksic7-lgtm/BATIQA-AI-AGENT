...
> **Update Mendalam 2026-08-21 (Try 2):** 8 URL dicek ulang via `webfetch` (semua `Transport error` — kemungkinan Cloudflare/bot block) + `websearch deep + livecrawl preferred` untuk `rooms`, `dining`, `facilities`, `meetings`, `suite`, `superior-room`. Hasil deep berhasil untuk **Dining** (`/dining` → FRESQA opening hours 06:00-23:00, signature Rendang Mintuo/Teh Talua, 7 menu), **Rooms** (`/room-type/suite` → 44 m² 32" TV, `126 Superior Rooms` highlight), dan **Facilities** via article. `photos`, `offers`, `insiders-guide` masih belum ada content publik.
...
| **4. ROOM TYPES** | Suite: **7 Suites**, **44 m²** (Official `room-type/suite` 44 m², 32" TV, sofa, living room, large windows) vs **45 m²** (Official article `liburan-asyik` & VenueMagz) — **CONFLICT 44 vs 45** (room-type/suite lebih spesifik) | Official `room-type/suite` HIGH (44 m²) + Official article 45 m² + Agoda 44 m² | HIGH (44 m² room-type) / CONFLICT |
| **4. ROOM TYPES** | Superior detail (deep): **22 sqm** warm wooden finishings, double or twin beds, **32" flat-screen TV** international channels, work desk, en-suite bathroom shower, complete amenities (dari Surabaya Superior sebagai referensi template BATIQA, tapi untuk Pekanbaru highlight: `126 Superior Rooms DOUBLE & TWIN BED` | Official BATIQA `hotels/pekanbaru` highlight (126 Superior) + `room-type/superior-room` Surabaya template 22 sqm (LOW untuk Pekanbaru) + `websearch deep` suite | MEDIUM |
...
| **7. DINING / RESTAURANT** | **FRESQA Bistro** — 1 restaurant utama, indoor & outdoor, **homey atmosphere, warm-colored artistic interior, cozy outdoor space**, **specialist international cuisine** (Indonesian, Asian, Western), **FRESQA = Fresh + BATIQA** fresh ingredients. **Opening Hours: 06:00 AM - 11:00 PM** (deep `dining` page), **Signature Dish: Rendang Mintuo** (slow-cooked beef coconut milk) & **Teh Talua** (West Sumatran tea egg yolk, sugar, lime). **Reserve:** `reservation.pekanbaru@batiqa.com` | Official `hotels/pekanbaru/dining` deep (HIGH) + JTB + Agoda | HIGH |
| **7. DINING / RESTAURANT** | **7 Signature Menu** 11 Aug 2026 + **Culinary Journey** (Patin Lancang Kuning + Es Air Mata Pengantin, Melayu modern, Fresqa Bistro & room service, `official whatsapp 0761889000`), **Japanese Street Food** (Curry Udon, Miso Ramen, Set Menu, Tempura, Miso Soup, teh ocha free refill, Rp 50rb, 11:00-14:00 & 16:00-21:00) — **EXTERNAL** tapi konsisten | Tribun/RiauPos/RiauAktual/JurnalPekan (GM Alpino) | MEDIUM |
...
**Catatan Deep Check 8 URL (2026-08-21 Try 2):**
- `webfetch` langsung ke `batiqa.com/hotels/pekanbaru/*` (rooms, facilities, dining, photos, offers, meetings, insiders-guide, main) semua `Transport error` — kemungkinan Cloudflare / bot protection.
- `websearch deep + livecrawl preferred` berhasil untuk: **Main** (126 Superior Rooms, 7 Suites highlight), **Rooms** (`room-type/suite` 44 m² 32" TV, `superior-room` Surabaya template 22 sqm), **Dining** (`FRESQA` opening hours 06:00-23:00, signature dish, 7 menu, Culinary Journey), **Facilities** via article `liburan-asyik` (4 meeting, FRESQA, gym, wifi). **Photos / Offers / Insiders-guide / Meetings** tidak ada content publik di index (hanya judul menu), sehingga tetap `UNKNOWN` — perlu HUMAN VERIFICATION atau akses via browser manual.
